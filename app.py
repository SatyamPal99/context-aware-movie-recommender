import time
from pathlib import Path    # better handiling with file paths

import pandas as pd
import streamlit as st      
from rapidfuzz import fuzz  # for approximate/wrong string matching(fix user typos in movie names).

from src.models.new_user_recommender import NewUserRecommender   # model for new users.
from src.models.existing_user_recommender import ExistingUserRecommender
 
from src.utils.time_utils import get_time_of_day        #morning / afternoon / night (context aware)
from src.explainability.explain import generate_explanation  #giving explaination for recommended movies.

# Real-time model updates and user interaction tracking functions (clicks, likes, watch later, etc)
...
TOP_N = 15   # top 15 recommendations per users


# -------------------------
# PAGE CONFIG (create dashboard) on browser.
# -------------------------
st.set_page_config(
...
# LOAD DATA (CACHED)
# -------------------------
@st.cache_data    # decorator in streamlit, runs function once stored it and reuse it.
def load_data():
    return pd.read_csv("data/processed/movie_features.csv") # load csv file of ALS in pandas dataframe(tabular format).

#loading results generated by ALS model .

@st.cache_data
def load_als_recommendations():
    if ALS_RECS_FILE.exists():
        return pd.read_csv(ALS_RECS_FILE)   # return als recommendations as pandas dataframe.
...
# -------------------------
# LOAD MODELS (CACHED) once and reuse for all.
# -------------------------
@st.cache_resource   # decorator in streamlit, runs function once stored it and reuse it.

# model_cache_key is caching and rebuilding models.
def load_models(movie_features, model_cache_key: str = "v2-negative-genres"):
    return (
        NewUserRecommender(movie_features),     #creates model for new users with no histroy.
        ExistingUserRecommender(movie_features) #creates model for existing users.
    )


movie_features = load_data()     #Loads the movie dataset (CSV) into a table (DataFrame)
als_recs = load_als_recommendations()  # loads precomputes ALS recommendations.
#loading cached models(reusing recommender model)
new_user_model, existing_user_model = load_models(movie_features, model_cache_key="v2-negative-genres")

# for explaining recommended movies to users .
...
# -------------------------
# for UI , creates spinner (loading animation)
# -------------------------
...
#This function displays a user’s watch history in the UI and lets them re-watch or like movies, 
# while also returning a clean list of recent movies

def show_history_section(user_id: str):
    recent_entries = get_recent_history_entries(user_id) #fetch detaild history of a user.

    st.subheader("📺 Previously Watched")  # creates a header in UI : previously watched

    if recent_entries:     #if user have history.
        for idx, item in enumerate(reversed(recent_entries), 1):
            movie = item.get("title") if isinstance(item, dict) else str(item)  #get movie title from history entry,if entry is dict.
            watched_at = item.get("watched_at") if isinstance(item, dict) else None   #watch time of movie.
            click_count = item.get("clicks") if isinstance(item, dict) else None     # numer of times user watched.

            col1, col2, col3 = st.columns([4, 1, 1])   #creater 3 col in UI .
            with col1:
...
# change recommendation ranking based on time and past history in that time slot.
...
#displays the user’s “Watch Later” list and lets them watch, remove, or like movies from it.
...
#This function fetches and returns top-N ALS recommendations for a specific user, sorted by score.
#check ALS data exist for user -> covert userid to int -> filter recommendations -> sort by score -> return top-N.
...
# this function maintains user profile based on ALS results by learning each user's preferences.
@st.cache_data
...
#this function find the best matching ALS user profile for a new user based on their input preferences and liked movies,
# it is hybrid filtering for to avoid cold start problem.
...
#takes a list of movie titles
#finds those movies in the dataset
#extracts their genres
#counts frequency of each genre
#returns a dictionary like
#count how many times each genre appears in given list of movies.
